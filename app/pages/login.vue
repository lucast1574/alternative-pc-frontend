<template>
  <div class="auth-page">
    <div class="auth-card">
      ...
      <p class="switch">¿No tienes cuenta? <NuxtLink to="/registro">Crear cuenta</NuxtLink></p>
    </div>
  </div>
</template>
...
async function handleLogin() {
  ...
  catch (e) { error.value = e.data?.error || 'Credenciales inválidas' }
...
.btn-w { display: block; padding: 12px; background: #fff; color: #000; border: none; border-radius: 8px; font-weight: 600; font-size: 0.9rem; cursor: pointer; width: 100%; text-align: center; }
.btn-w:hover { background: #ddd; }
.btn-w:disabled { opacity: 0.3; }
.btn-google {
  display: flex; align-items: center; justify-content: center; gap: 8px;
  width: 100%; padding: 12px; border-radius: 8px; background: var(--bg-elevated);
  border: 1px solid var(--border); color: var(--text); cursor: pointer; font-size: 0.9rem;
}
